...
本ドキュメントは、**SpeakFlow (AI 英会話・シャドーイング・瞬間英作文 トレーニング)** プロジェクトにおける設計原則、コード品質、命名規則、および開発プロセスに関する統一ガイドラインです。  
新規機能の開発、リファクタリング、およびコードレビュー時は本規約に準拠してください。

> 📖 **関連ドキュメント**:
> - 機能仕様・学習ガイド: [docs/FEATURES.md](./FEATURES.md)
> - システムアーキテクチャ・技術仕様: [docs/ARCHITECTURE.md](./ARCHITECTURE.md)
